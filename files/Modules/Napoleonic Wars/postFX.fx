...
float3 tonemapping(const float3 scene_color, const float2 luminanceAvgMax, const int tonemapOp) {
	...
	float3 final_color;
	{
     ...
     grey = dot(grey,scene_color_exposed).xxx;
     ...
     float3 scene_color_tonescaled = scene_color_exposed * toneScalar;
     
     scene_color_tonescaled = lerp(scene_color_tonescaled, grey, 0.450);
     scene_color_exposed = lerp(scene_color_exposed, scene_color_tonescaled, 0.450);
		 
     final_color = saturate(scene_color_exposed);
		}
	}
...
float4 FinalScenePassPS(uniform const bool use_dof, uniform const int use_hdr, uniform const bool use_auto_exp, float2 texCoord: TEXCOORD0) : COLOR {
    ...
    luminanceAvgMax = float2(0.5, 10.2);
		...
		if(use_auto_exp) {
      color.rgb = tonemapping(color.rgb, luminanceAvgMax, 3);
		}
		else {
      color.rgb = tonemapping(color.rgb, luminanceAvgMax, 0);
		}
	}
	else {
		color = scene;
